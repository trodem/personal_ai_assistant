...
- Bootstrapped Flutter clean architecture baseline in `mobile/` with fixed state management (`ChangeNotifier` + `InheritedNotifier`), introducing modular app layers (`core/state`, `presentation`, `screens`, `widgets`, `theme`) and controller tests for deterministic app-state behavior.
- Added mobile Supabase Auth login/logout flow with modular clean-architecture layers (`features/auth/domain|data|application|presentation`), async session restore at app bootstrap, and widget tests covering sign-in/sign-out behavior via deterministic fake auth repository.
- Added first-run onboarding focused on immediate value (`first memory` + `first question`) via modular `features/onboarding` controller/screen flow, with gated authenticated routing before main app surface and dedicated onboarding test coverage.
- Added onboarding step 1 screen with explicit welcome, value proposition, and privacy short notice; wired it as mandatory first onboarding stage before first-value flow and covered transition in auth/onboarding widget tests.
- Added onboarding step 2 language selection (`en`/`it`/`de`) with persistence contract to backend `preferred_language` via `PATCH /api/v1/me/settings/profile`, plus repository abstraction/fallback and automated widget/unit tests for language-save progression.
- Added onboarding step 3 permission guidance flow with explicit permission requests (`microphone` required, `camera` optional), modular permission gateway abstraction, and enforced progression gating before first-value onboarding.
- Added onboarding step 4 guided first-memory capture with explicit `Confirm / Modify / Cancel` proposal actions in the frontend flow, including controller state transitions and widget/unit regression coverage for proposal-confirmation behavior.
...
- Extended semantic cache invalidation to memory delete (`DELETE /api/v1/memory/{id}`) and added regression coverage for create/delete invalidation behavior in semantic caching policy tests.
- Added minimal question answer-context builder (`backend/app/services/question_context_builder.py`) to keep answer-generation context compact (bounded source IDs, compact totals, truncated semantic snippets) and wired it into `/api/v1/question` token/cost estimation path, with dedicated unit tests.
- Added dedicated natural-language answer generation service (`backend/app/services/question_answer_generation.py`) that renders final user-facing responses from structured backend question results; updated question route integration and added unit tests for key response paths.
- Hardened question-flow moderation by enforcing post-generation output safety checks also on semantic-cache hits, with dedicated regression tests for generated and cached unsafe-answer blocking.
- Added explicit retrieval-priority regression coverage for question flow to enforce database-first behavior: structured queries bypass semantic fallback, while open-ended no-result queries invoke semantic retrieval as fallback.
...
### Changed
- Completed P7 task `Centralize Flutter style tokens (colors, typography, spacing) and enforce usage across all screens`: added semantic UI tokens (`onAccent`, `danger`) and shared text-style tokens (`AppTextStyles`), then replaced remaining hardcoded UI error/text colors and inline styles across auth/onboarding/chat widgets to enforce token-based styling.
- Completed P5 task `Enforce 2FA policy by role`: enforced privileged-role invariant in security settings by blocking `disable_2fa` for `admin`/`author` (`403 auth.mfa_required`), while preserving optional 2FA behavior for `user`; added dedicated role-policy regression tests for admin/author disable denial and user no-2FA access to non-privileged endpoint.
- Completed P5 task `Add role-based access control (user, admin, author) and enforce on privileged endpoints`: enforced privileged-route role checks (`admin`/`author` on admin paths, `author` only on author paths), kept billing-role lock constraints aligned with RBAC policy, and validated behavior with dedicated RBAC matrix and privileged endpoint regression suites.
- Completed P5 task `Add tenant-aware authorization guardrails for future B2B mode (tenant_id context propagation + checks)`: hardened tenant guardrails in auth by normalizing tenant header checks, requiring tenant context for tenant-scoped tokens, rejecting cross-tenant header mismatches, and blocking custom tenant headers for single-tenant fallback tokens; added regression tests for auth-level tenant checks and privileged endpoint tenant enforcement.
- Completed P5 task `Add user account status control (active, suspended, canceled) and block access when suspended/canceled`: made account status enforcement authoritative at auth time by checking persisted tenant-scoped user status before request access, and added endpoint regression tests proving users marked `suspended` or `canceled` via admin status endpoint are blocked (`403 auth.forbidden`) on protected routes.
- Completed P5 task `Enforce author safety invariants`: enforced invariant guards on privileged user-status updates so `author` cannot suspend/cancel own account and operations that would remove the last active author are rejected (`422 auth.last_active_author_required`), while preserving existing self-role-change protection on author role endpoint; added dedicated regression tests for both invariants.
- Completed P5 task `Negative auth tests`: added consolidated negative-auth contract coverage for missing/expired token, missing/invalid 2FA verification code on security action, privileged access blocked without 2FA, cross-user resource access rejection, suspended/canceled user blocking, non-admin/non-author privileged route denial, admin role-change denial on author-only endpoint, and premium+billing-exempt enforcement for admin/author role assignment paths.
- Completed P6 task `Align question behaviors and edge cases with docs/query-contract.md`: added dedicated query-contract regression suite covering ambiguous-last handling (`422 query.ambiguous_intent`), deterministic latest lookup provenance, multi-currency separation (no implicit conversion), no-result Add Memory fallback, out-of-scope boundary response, language fallback behavior, and SSE terminal `done` event metadata contract (`confidence`, `source_memory_ids`).
- Completed P6 task `Equivalent text-query endpoint for testing and fallback`: added explicit contract tests for `/api/v1/question` as canonical non-stream fallback, including response-schema assertions, equivalence with `/api/v1/voice/question` for the same prompt, and validated fallback flow from `/api/v1/question/stream` `503 ai.provider_unavailable` to successful `/api/v1/question` response.
- Completed P6 task `Intent detection oriented to SQL queries/aggregations`: introduced dedicated query-intent detection module with SQL-oriented metadata (`aggregation`, `target`, `order_by_when_desc`, `limit`, `currency_constraint`) and integrated it into the question engine while preserving API behavior; added focused intent-detection unit tests and kept existing question-flow regressions green.
- Completed P6 task `Enforce strict rule: calculations in backend, never in LLM`: aligned question-answer generation telemetry to explicit deterministic backend execution (`provider=backend`, `model_id=deterministic-question-engine`) and added enforcement regression test ensuring `/api/v1/question` runs under backend-calculation labeling with no OpenAI provider series for `answer_generation`.
- Completed P6 task `Implement deterministic rule for "latest/last": ORDER BY when DESC LIMIT 1`: updated latest-record selection in question engine to sort by normalized `structured_data.when` descending with deterministic `created_at` tie-breaker, and added regression test proving latest lookup follows `when` chronology rather than insertion order.
- Completed P6 task `Implement ambiguity handling with clarification question before final answer`: standardized ambiguous-query response (`422 query.ambiguous_intent`) to include explicit clarification payload details (`clarification_question` and one-item `clarification_questions` list), and added dedicated text/voice endpoint tests to enforce one-question-per-turn clarification behavior before final answer generation.
- Completed P6 task `Implement no-result fallback response (no fabrication)`: added explicit no-result fallback contract tests for both text and voice question endpoints to enforce low-confidence empty provenance (`source_memory_ids=[]`), Add Memory suggestion in response copy, and absence of fabricated numeric currency amounts when no relevant memories exist.
- Completed P6 task `Define and enforce multi-currency policy (no silent conversion; explicit aggregation rules by currency)`: added dedicated policy tests for text and voice question flows to enforce currency-separated totals, prevent implicit cross-currency sum conversion, and verify explicit currency-filter behavior (`... in CHF`) returns only the requested currency aggregate.
- Completed P6 task `Core aggregate queries: expenses by period/category/object`: extended expense aggregation pipeline with deterministic backend filters for period (`today/this_month/last_month/this_year/last_year`) and object/category constraints extracted from query text (for example `on the motorcycle`, `in category transport`), with dedicated regression coverage for period-scoped, object-scoped, and category-scoped expense totals.
- Completed P6 task `Core aggregate queries: loan balances (who owes what)`: added deterministic loan-balance query intent and backend aggregation over `loan_event` records by counterparty/currency/action, including support for receivable-only phrasing (`who owes me`) and full balance phrasing (`who owes what`), with dedicated regression tests for success and no-result fallback behavior.
- Completed P6 task `Core aggregate queries: inventory state from events`: added deterministic inventory-state query intent and backend event-sourcing aggregation over `inventory_event` records (`add`/`remove`) with item/location filters (for example `How many peas are left in the cellar?`), including regression tests for add-remove net state, location scoping, and no-result fallback behavior.
- Completed P6 task `Semantic retrieval with pgvector for open-ended questions`: added tenant/user-scoped semantic retrieval fallback for open-ended queries by combining deterministic embedding similarity with lexical overlap over stored memory embeddings, integrated only after structured query paths return no-result, and added regression tests for location-recall retrieval plus database-first priority preservation on structured queries.
- Completed P6 task `Implement semantic answer cache with user-scoped keying and similarity threshold`: added dedicated regression coverage on open-ended semantic question flow to verify per-user cache isolation and threshold-gated reuse behavior (`0.90` similarity policy), including positive hit for near-identical query and negative hit (new cache entry) for semantically different query.
- Completed P5 task `Implement 2FA/TOTP enrollment and verification flow in security settings`: added security-flow handling for `enable_2fa`, `disable_2fa`, and `verify_2fa` actions with per-user pending challenge state, explicit `totp_code` validation, deterministic MVP TOTP verification, and immediate `mfa_enabled` state reflection in `/api/v1/me/settings`; added dedicated endpoint regression tests for success and failure paths.
...
- Reworked API backward-compatibility coverage in `backend/tests/test_api_backward_compatibility.py` with deterministic `TestClient` contract checks for both previous and active supported clients (v1-only path publication, memories fields, question response fields, settings fields, and admin users fields), removing dependency on externally running localhost server; marked the related P4 TODO task as completed.
- Expanded streaming API regression tests (`backend/tests/test_question_streaming.py`) to explicitly verify chunk ordering, single terminal `done` event semantics, reconstruction parity with non-stream `/api/v1/question`, and practical fallback flow after stream failure (`503` -> non-stream success); marked the related P4 TODO task as completed.
- Added dedicated API contract regression suite for core success/error responses (`backend/tests/test_api_contract_core_responses.py`) covering: save-memory success payload shape, `422 memory.missing_required_fields`, `422 memory.confirmation_required`, and standard `401 auth.missing_token` error schema on protected core endpoint; marked the related P4 TODO task as completed.
- Completed P4 required-fields error contract for `POST /api/v1/memory`: incomplete required payloads return `422 memory.missing_required_fields` with explicit missing-field details; added OpenAPI regression assertion for the save-memory `422` response and marked the related TODO task as completed.
...
- Implemented `POST /api/v1/notifications/{id}/read` with typed `{updated: true}` response, user-scoped read-state update, and `404` handling for missing notifications; added endpoint/OpenAPI regression tests and marked the related P4 TODO task as completed.
